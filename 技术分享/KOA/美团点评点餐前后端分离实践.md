...
<!-- 点餐终端团队在2017年Q2启动了商家平台前后端分离的建设工作，按照新技术先小范围试点再逐步推广到整条业务线的模式，我们开始在监控平台搭设自己的前端服务系统。 -->
### 一、什么是前后端分离
...
当客户端发起请求的时候，Node服务器会先由主路由文件作为请求入口，将请求分发给各个子路由。子路由在监听到对应的HTTP请求后，会向对应的控制器请求数据。控制器访问不同的数据Model，并对各个Model返回的数据进行统一处理与封装。之后将处理后的数据返回给路由，最终响应客户端的请求。一个标准的请求与响应过程如图2所示：
...
这里提一下样例里使用的一个Koa日志中间件Koa-logger，该中间件可以在开发的过程中实时监听请求，并在控制台实时打印日志。启用也十分方便，只需在项目入口文件app.js里配置"app.use(logger());"命令即可。其打印日志效果图如下所示：
